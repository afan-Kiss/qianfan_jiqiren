#!/usr/bin/env node
/**
 * 抖店向指定买家发送消息（UI 点击，需 --confirm-send）
 * npm run doudian:send-message-to-buyer -- --buyer-name "一只小青蛙" --text "你好" --confirm-send
 */
...
async function main() {
  ...
  const cli = parseSendMessageCliArgs(argv);
  const cfg = getDoudianConfig();

  console.log('=== 抖店向买家发送消息 ===');
  console.log(`目标买家: ${cli.buyerName}`);
  ...
  if (!cli.confirmSend) {
    const report = await runMessageSendSession({
      targetBuyerName: cli.buyerName,
      text: cli.text,
      confirmSend: false,
      mockMode: false,
    });
    const paths = writeReports(report, {
      prefix: 'doudian-send-message-to-buyer',
      buildTextReport: buildSendMessageTextReport,
    });
    console.log('\n' + buildSendMessageTextReport(report).join('\n'));
  ...
  const { runLock, portGuard } = await beginDoudianLiveRun({
    command: 'doudian:send-message-to-buyer',
    argv,
    port: cfg.bridgePort || 19527,
    reportPrefix: 'doudian-send-message-to-buyer',
    buildTextReport: buildSendMessageTextReport,
  });
...
main().catch((err) => {
  console.error('send-message-to-buyer 异常:', err.message || err);
  process.exit(1);
});
