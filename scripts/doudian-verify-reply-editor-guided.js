...
/**
 * 抖店引导式客服输入框诊断
 * npm run doudian:verify-reply-editor-guided
 * npm run doudian:verify-reply-editor-guided -- --timeout-minutes 60
 */
...
async function main() {
  ...
  console.log('=== 抖店引导式客服输入框诊断 ===');
  console.log('本命令持续等待您手动点开买家会话，检测输入框与发送按钮，不填内容、不发送');
  ...
  const { runLock, portGuard } = await beginDoudianLiveRun({
    command: 'doudian:verify-reply-editor-guided',
    argv: process.argv.slice(2),
    port: cfg.bridgePort || 19527,
    reportPrefix: 'doudian-reply-editor-guided',
    buildTextReport: buildGuidedReplyEditorTextReport,
  });
  ...
  const paths = writeReports(report, {
    prefix: 'doudian-reply-editor-guided',
    buildTextReport: buildGuidedReplyEditorTextReport,
  });

  console.log('\n' + buildGuidedReplyEditorTextReport(report).join('\n'));
...
main().catch((err) => {
  console.error('verify-reply-editor-guided 异常:', err.message || err);
  process.exit(1);
});
