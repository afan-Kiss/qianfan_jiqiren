...
/**
 * 抖店引导式草稿填入输入框
 * npm run doudian:fill-reply-draft-guided
 * npm run doudian:fill-reply-draft-guided -- --timeout-minutes 60
 */
...
async function main() {
  ...
  console.log('=== 抖店引导式草稿填入输入框 ===');
  console.log('本命令持续等待您手动点开草稿对应的买家会话，填入 draft_only 草稿，不点击发送');
  ...
  const { runLock, portGuard } = await beginDoudianLiveRun({
    command: 'doudian:fill-reply-draft-guided',
    argv: process.argv.slice(2),
    port: cfg.bridgePort || 19527,
    reportPrefix: 'doudian-fill-reply-draft-guided',
    buildTextReport: buildGuidedFillDraftTextReport,
  });
  ...
  const paths = writeReports(report, {
    prefix: 'doudian-fill-reply-draft-guided',
    buildTextReport: buildGuidedFillDraftTextReport,
  });

  console.log('\n' + buildGuidedFillDraftTextReport(report).join('\n'));
...
main().catch((err) => {
  console.error('fill-reply-draft-guided 异常:', err.message || err);
  process.exit(1);
});
