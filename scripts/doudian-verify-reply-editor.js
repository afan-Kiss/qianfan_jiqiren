...
/**
 * 抖店客服输入框诊断（不填内容、不发送）
 * npm run doudian:verify-reply-editor
 */
...
async function main() {
  ...
  console.log('=== 抖店客服输入框诊断 ===');
  console.log('本命令仅检测输入框与发送按钮，不填内容、不发送');
  console.log(`超时: ${timeoutMinutes} 分钟`);
  console.log('请在 IM 打开后手动选中买家会话');
  ...
  const paths = writeReports(report, {
    prefix: 'doudian-reply-editor',
    buildTextReport: buildReplyEditorTextReport,
  });

  console.log('\n' + buildReplyEditorTextReport(report).join('\n'));
...
main().catch((err) => {
  console.error('verify-reply-editor 异常:', err.message || err);
  process.exit(1);
});
